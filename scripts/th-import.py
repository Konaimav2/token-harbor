...
ROUTER_BASE_DEFAULT = "http://localhost:20128"
...
def infolog(msg): print(f"  [*] {msg}", flush=True)
def warnlog(msg): print(f"  [!] {msg}", flush=True)
def elog(msg, detail=""):
...
def main():
    ...
    imported = set() if args.force else load_imported()
    pending = [(e, k) for e, k in keys if _md5(k) not in imported]
    oklog(f"Belum di-import: {len(pending)}")
    ...
    for email, key in pending:
        ...
        if status == "available":
            ...
            warnlog(f"{email}: RATELIMITED ({detail})")
        else:
    ...
    # NEVER import not_verified / ratelimited keys — only truly available ones.
    pending = available
    if not pending:
    ...
    for i, (email, key) in enumerate(pending, start_conn):
        used_names.add(name := f"{args.prefix} {i}")
        ...
        try:
            ...
            elog(f"[{i}] POST gagal: {email[:20]}", traceback.format_exc()[:200])
            fail += 1
            continue
        ...
        if resp.status_code in (200, 201):
            ...
            oklog(f"[{i}] {name} -> connection {cid} (prio {priority})")
        else:
            try:
                err = resp.json().get("error") or resp.text[:200]
            except Exception:
                err = resp.text[:200]
            elog(f"[{i}] DITOLAK: {email[:20]}", err)
            fail += 1
